...
export class ReportviewComponent implements OnInit {
  ...
  prepareDataHeader() {
    switch (parseInt(this.inputParameters.TYPE, 10)) {
      case RESPONSE_TYPE.YEARLY:  //this.monthViewReport = false;
      case RESPONSE_TYPE.MONTHLY: //this.monthViewReport = true;
        this.clipHeaders();
        break;
      case RESPONSE_TYPE.QUATERLY:
        this.clipHeaders();
        break;
    }
    this.startDate = new Date(this.inputParameters.YEAR, 0,1) ;
  ...
  clipHeaders() {
    ...
    if(this.inputParameters.TYPE === RESPONSE_TYPE.QUATERLY) {
      let clipper = (this.inputParameters.QUARTER_VIEW.QUARTER_NUMBER - 1) * 3;
      for( let i = 0 ;i < 12; i++) {
    ...
    //console.log(this.dataHedersToShow);
  }
  fillDateHeader() {
